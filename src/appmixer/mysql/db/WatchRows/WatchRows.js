...
module.exports = {
    ...
    async test(context) {
        ...
        // instead of writing them to the data store. There is no historical old value to compare
        // against in Test Mode, so we emit the newest row currently returned by the query in the
        // SAME shape receive() emits on the 'out' port for an "insert" event.
        const row = await fetchLatestRow(context, query, [], idField);
        if (!row) {
            throw new Error('No rows returned by the query to use as test data.');
        }

        return context.sendJson({ event: 'insert', newRow: row }, 'out');
    }
};
