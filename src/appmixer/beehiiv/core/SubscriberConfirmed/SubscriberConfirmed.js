...
module.exports = {
    ...
    async test(context) {
        const record = await lib.fetchLatestSubscription(context, { status: 'active' });
        if (!record) {
            throw new Error('No recent confirmed (active) subscriptions to use as test data.');
        }
        return context.sendJson(lib.toWebhookShape(record, 'subscription.confirmed'), 'out');
    }
};
