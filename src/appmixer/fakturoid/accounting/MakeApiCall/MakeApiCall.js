...
module.exports = {
    async receive(context) {
        ...
        const baseUrl = `https://app.fakturoid.cz/api/v2/accounts/${context.auth.slug}`;
        const targetUrl = url.startsWith('http://') || url.startsWith('https://')
            ? url
            : `${baseUrl}${url.startsWith('/') ? url : '/' + url}`;

        const basicAuth = Buffer.from(`${context.auth.email}:${context.auth.apiKey}`).toString('base64');
