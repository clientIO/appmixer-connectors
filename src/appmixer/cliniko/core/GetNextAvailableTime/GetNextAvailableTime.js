...
module.exports = {
    ...
    async receive(context) {
        ...
        const path = buildPath(context, content, 'next_available_time');
        ...
        if (!data || !data.appointment_start) {
            throw new context.CancelError(
                'No available time was found for this practitioner, business and appointment type in the given window.'
            );
        }

        return context.sendJson({ appointment_start: data.appointment_start }, 'out');
    }
};
