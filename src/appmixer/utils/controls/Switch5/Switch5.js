...
module.exports = {
    ...
    receive(context) {
        ...
        for (let n = 1; n <= 9; n++) {
            ...
            let expValid = true;
            if (!context.messages.in.content[`expression${n}`]) {
                continue;
            }

            for (let i in context.messages.in.content[`expression${n}`]['AND']) {
                // Uses the same logic as utils/controls/Condition/Condition.js
                let expressionAnd = context.messages.in.content[`expression${n}`]['AND'][i];
            ...
            if (expValid) {
                return context.sendJson({}, `out${n}`);
            }
        }

        return context.sendJson({}, 'else');
    }
};
