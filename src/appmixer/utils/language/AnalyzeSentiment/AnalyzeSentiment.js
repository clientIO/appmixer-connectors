...
module.exports = {
    ...
    receive(context) {
        ...
        let { document, type = 'PLAIN_TEXT' } = context.messages.document.content;
        ...
        return client
            .analyzeSentiment({
                document: { content: document, type: type },
                encodingType: 'UTF8'
            })
            .then(results => {
                const sentiment = results[0].documentSentiment;
                return context.sendJson({ score: sentiment.score, magnitude: sentiment.magnitude }, 'sentiment');
            });
    }
};
