...
module.exports = {
    ...
    receive(context) {
        ...
        const url = `https://api.sageone.com/accounts/v1/products/${product.productId}`;
        const data = buildProduct(product);

        return commons.sageoneAPI('PUT', token, url, userAgent, clientSigningSecret, data)
            .then(product => {
                const data = JSON.parse(product);
                return context.sendJson(data, 'updatedProduct');
            });
    }
};
